...
**Monorepo (Notes - Blackwall):** from the workspace root:

```bash
git remote add origin https://github.com/kineticdirt/Notes-Blackwall.git
...
**Quick setup (paste token once):** From the repo root, run `./DaDumpMan/scripts/setup-github-pat.sh`. When prompted, paste your PAT (input is hidden). It sets origin to `Notes-Blackwall` by default; then run `git push -u origin main`. For a different repo: `REPO_NAME=DaDumpMan ./DaDumpMan/scripts/setup-github-pat.sh`. Different user: `GITHUB_USER=youruser ./DaDumpMan/scripts/setup-github-pat.sh`.
...
Remote must be HTTPS (not SSH). Use `Notes-Blackwall` for the monorepo, or the repo name if pushing a standalone project:
...
- **Use HTTPS, not SSH.** The PAT script stores credentials for HTTPS. If your remote is `git@github.com:...`, git uses SSH (your key), not the PAT. Fix: `git remote set-url origin https://github.com/kineticdirt/Notes-Blackwall.git`
